...
import type {
  Product, Category, Sale, Customer, Expense, Supplier, Feedback, MoneyAccount, MoneyTransaction, UserProfile
} from '@/types';
...
export function useCategories() {
  ...
  const addCategory = useCallback((cat: Omit<Category, 'id' | 'createdAt'>) => {
    const newCat: Category = { ...cat, id: generateId(), createdAt: new Date().toISOString() };
  ...
  const deleteCategory = useCallback((id: string) => {
    ...
    setItem(DB_KEYS.products, products.filter(p => p.categoryId !== id));
  }, []);
...
export function useProducts() {
  ...
  const addProduct = useCallback((prod: Omit<Product, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    const newProd: Product = { ...prod, id: generateId(), createdAt: now, updatedAt: now };
  ...
  const deleteProduct = useCallback((id: string) => {
    ...
    setItem(DB_KEYS.sales, sales.filter(s => s.items.every(i => i.productId !== id)));
  }, []);
...
export function useSales() {
  ...
  const addSale = useCallback((sale: Omit<Sale, 'id' | 'createdAt' | 'receiptNumber'>) => {
    ...
    if (cashAccount) {
      const transactions = getItem<MoneyTransaction[]>(DB_KEYS.moneyTransactions, []);
      transactions.push({
        id: generateId(),
...
export function useCustomers() {
  ...
  const addCustomer = useCallback((cust: Omit<Customer, 'id' | 'createdAt' | 'totalPurchases' | 'totalSpent' | 'lastPurchaseDate'>) => {
...
export function useExpenses() {
  ...
  const addExpense = useCallback((exp: Omit<Expense, 'id' | 'createdAt'>) => {
    const newExp: Expense = { ...exp, id: generateId(), createdAt: new Date().toISOString() };
...
export function useSuppliers() {
  ...
  const addSupplier = useCallback((sup: Omit<Supplier, 'id' | 'createdAt' | 'totalSupplies'>) => {
    const newSup: Supplier = { ...sup, id: generateId(), totalSupplies: 0, createdAt: new Date().toISOString() };
...
export function useFeedback() {
  ...
  const addFeedback = useCallback((fb: Omit<Feedback, 'id' | 'createdAt'>) => {
    const newFb: Feedback = { ...fb, id: generateId(), createdAt: new Date().toISOString() };
...
export function useMoneyAccounts() {
  const [accounts, setAccounts] = useState<MoneyAccount[]>(() => {
    ...
    if (stored.length === 0) {
      const defaultAccounts: MoneyAccount[] = [
        { id: generateId(), name: 'Cash in Hand', type: 'cash', balance: 0, description: 'Main cash register', createdAt: new Date().toISOString() },
        { id: generateId(), name: 'Bank Account', type: 'bank', balance: 0, description: 'Primary bank account', createdAt: new Date().toISOString() },
        { id: generateId(), name: 'Mobile Money', type: 'mobile', balance: 0, description: 'Mobile payment wallet', createdAt: new Date().toISOString() },
      ];
  ...
  const addAccount = useCallback((acc: Omit<MoneyAccount, 'id' | 'createdAt'>) => {
    const newAcc: MoneyAccount = { ...acc, id: generateId(), createdAt: new Date().toISOString() };
    setAccounts(prev => [...prev, newAcc]);
    return newAcc;
  }, []);

  const updateAccount = useCallback((id: string, updates: Partial<MoneyAccount>) => {
...
export function useMoneyTransactions() {
  ...
  const addTransaction = useCallback((tx: Omit<MoneyTransaction, 'id' | 'createdAt'>) => {
    const newTx: MoneyTransaction = { ...tx, id: generateId(), createdAt: new Date().toISOString() };
...
export function useAnalytics() {
  ...
  const getSalesChartData = useCallback((range: string) => {
    const sales = getSalesByDateRange(range);
    const data: Record<string, { name: string; sales: number; expenses: number; profit: number }> = {};

    sales.forEach(s => {
      const date = new Date(s.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
      if (!data[date]) data[date] = { name: date, sales: 0, expenses: 0, profit: 0 };
      data[date].sales += s.total;
      data[date].profit += (s.total - s.items.reduce((sum, i) => sum + (i.unitPrice * 0.6 * i.quantity), 0));
    });
    ...
    sales.forEach(s => {
      s.items.forEach(i => {
        if (!productSales[i.productName]) {
          productSales[i.productName] = { name: i.productName, quantity: 0, revenue: 0 };
        }
  ...
  return { getSalesByDateRange, getKPIs, getSalesChartData, getTopProducts, getRecentSales };
}
